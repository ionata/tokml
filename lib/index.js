      ...
      if (styleHash) {
        ...
        if (validators.isPoint(_.geometry) && hasMarkerStyle(properties)) {
          if (styleHashesArray.indexOf(styleHash) === -1) {
            styleContents += markerStyle(properties);
          ...
          if (styleHashesArray.indexOf(styleHash) === -1) {
            styleContents += polygonAndLineStyle(properties);
            styleHashesArray.push(styleHash);
          }
          styleReference = tag('styleUrl', '#' + styleHash);
        ...
        if (styleContents) styleDefinition = style(styleHash, styleContents);
      }
    }
 ...
 * @returns {string}
 */
function style(styleHash, contents) {
  return tag('Style', { id: styleHash }, contents);
}
 ...
 * @returns {string}
 */
function markerStyle(_) {
  return tag('IconStyle', tag('Icon', tag('href', iconUrl(_)))) + iconSize(_);
}
 ...
 * @returns {string}
 */
function polygonAndLineStyle(_) {
  const lineStyle = tag(
    'LineStyle',
